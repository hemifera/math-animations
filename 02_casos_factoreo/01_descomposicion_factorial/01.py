...
class LinePoints:
    def __init__(self, table: MathTable, row_size, col_size):
        top = (
            table.get_entries((1, row_size - 1)).get_top()
            + table.get_entries((1, row_size)).get_top()
        ) / 2

        if row_size == 1:
            bottom = np.array(
                [top[0], table.get_entries((col_size, row_size)).get_bottom()[1], 0]
            )
        else:
            bottom = np.array(
                [top[0], table.get_entries((col_size, row_size - 1)).get_bottom()[1], 0]
            )
...
class fc_01(Scene):
    def construct(self):
        ...
        f21 = MathTex(
            r"2 \cdot {{1}} + 2 \cdot {{3}} + 2 \cdot {{5}}", substrings_to_isolate="2"
        )
        ...
        self.play(FadeOut(ft3), ft4.animate.arrange(DOWN, buff=0.3).move_to(ORIGIN))
        self.wait(1)
        ...
        f41 = (
            MathTex(
                r"2\cdot {{a}} + 2 \cdot {{b}} + 2 \cdot {{c}}",
                substrings_to_isolate="2",
            )
            .move_to(f32.get_center())
            .set_color_by_tex("a", RED)
            .set_color_by_tex("b", RED)
            .set_color_by_tex("c", RED)
        )
        f42 = (
            MathTex(r" 2 ( {{a}} + {{b}} + {{c}} )", substrings_to_isolate="2")
            .move_to(f33.get_center())
            .set_color_by_tex("a", RED)
            .set_color_by_tex("b", RED)
            .set_color_by_tex("c", RED)
        )
        ...
        self.play(TransformMatchingTex(Group(f32, variables), f41))
        self.wait(1)
        self.play(TransformMatchingTex(Group(f33, variables), f42))
        self.wait(1)
...
class ejemplo_uno(Scene):
    def construct(self):
        ...
        t0 = MathTable([[2, 6, 10, 2]], line_config={"stroke_width": 0})
        t1 = MathTable([[2, 6, 10, 2], [1, 3, 5, 0]], line_config={"stroke_width": 0})

        lp0 = LinePoints(t0, 4, 1)
        lp1 = LinePoints(t1, 4, 2)
        self.add(t0.get_entries()[3].set_opacity(0), t1.get_entries()[7].set_opacity(0))
        self.play(Create(lp0.line), Create(t0))
...
class ejemplo_tres(Scene):
    def construct(self):
        ...
        tabla = t4
        table = MathTable(tabla, line_config={"stroke_width": 0}, v_buff=0.5)
        ...
        f0 = MathTex(
            r"{{30}}a+{{40}}b+{{80}}c", substrings_to_isolate=["30", "40", "80"]
        )
        self.play(Write(f0))
        self.wait(1)
        self.play(f0.animate.shift(UP * 3))
        self.wait(1)
        self.play(TransformMatchingTex(f0.copy(), tf1))
        self.wait(1)
        ...
        for i, fila in enumerate(filas[1:]):
            ...
            if i + 1 != len(filas[1:]):
                self.play(
                    f_non_zero[-1].animate.set_opacity(1),
                )
                self.wait(2)
            self.wait(2)
        ...
        self.play(TransformMatchingTex(tc1, f2))

        self.wait(1)
        self.play(
            TransformMatchingShapes(
                f2, f3, key_map={"{{2}}\times {{5}} ": "10"}, path_arc=PI / 2
            )
        )
        self.wait(1)
...
class ejemplo_cuatro(Scene):
    def construct(self):
        ...
        f0 = MathTex(
            r"{{42}}m^{3}+{{105}}m^{4}+{{168}}m^{5}",
            substrings_to_isolate=["42", "105", "147"],
        )
        f1 = MathTex(
            r"{{42}}m^{3}+{{105}}m^{3} \cdot m^{1} +{{168}}m^{3} \cdot m^{2}",
            substrings_to_isolate=["42", "105", "147"],
        )
        f2 = MathTex(
            r"m^{3}({{42}}+{{105}}m^{1} + {{168}}m^{2})",
            substrings_to_isolate=["42", "105", "147"],
        )
        f3 = MathTex(
            r"m^{3}({{42}}+{{105}}m + {{168}}m^{2})",
            substrings_to_isolate=["42", "105", "147"],
        )
        self.play(Write(f0))
        self.wait(1)
        ...
        self.wait(1)
        self.play(
            TransformMatchingShapes(f1, f2, path_arc=PI / 2, key_map={"m^{3}": "m^{3}"})
        )
        self.wait(1)
        ...
        self.wait(1)
        self.play(TransformMatchingTex(f3.copy(), tf1))
        self.wait(1)
        ...
        [tc1.add(element.copy()) for element in columnas[-1][0:-1]]
        [tc1.add(element.copy()) for element in filas[-1][0:-1]]
        f4 = MathTex(r" {{3}}\cdot {{7}} \cdot m^{3} ({{2}}+{{5}}m+{{8}}m^{2}) ").shift(
            DOWN * 2.5
        )
        f5 = MathTex(r"21m^{3}(2+5m+8m^{2}) ").shift(DOWN * 2.5)
        self.play(TransformMatchingTex(tc1, f4))

        self.wait(1)
        self.play(
            TransformMatchingShapes(
                f4, f5, key_map={"{{3}}\times {{7}} ": "21"}, path_arc=PI / 2
            )
        )
        self.wait(1)
...
class ejemplo_cinco(Scene):
    def construct(self):
        ...
        f1 = MathTex(
            r"{{150}}a^{2} b^{2} +{{210}}a^{2} b^{2} \cdot b^{2} + {{270}}a^{2} b^{2} \cdot b^{4}"
        )
        f2 = MathTex(r"a^{2} b^{2}({{150}}+{{210}}b^{2} + {{270}}b^{4})")
        f3 = MathTex(
            r"a^{2} b^{2}({{150}}+{{210}}b^{2} + {{270}}b^{4})",
            substrings_to_isolate=["42", "105", "147"],
        )
        self.play(Write(f0))
        self.wait(1)
        self.play(
            TransformMatchingShapes(
                f0, f1, path_arc=PI, key_map={r"a^{2} b^{2}": r"a^{2} b^{2}"}
            )
        )
        self.wait(1)
        self.play(
            TransformMatchingShapes(
                f1, f2, path_arc=PI, key_map={r"a^{2} b^{2}": r"a^{2} b^{2}"}
            )
        )
        self.wait(1)
        self.play(TransformMatchingShapes(f2, f3))
        ...
        [tc1.add(element.copy()) for element in filas[-1][0:-1]]
        f4 = MathTex(
            r" {{2}}\cdot {{3}}\cdot {{5}} \cdot a^{2} b^{2}({{5}}+{{7}}b^{2} + {{9}}b^{4})"
        ).shift(DOWN * 2.5)
        ...
        self.play(
            TransformMatchingShapes(
                f4, f5, key_map={r"{{2}}\cdot {{3}}\cdot {{5}}": r"30"}, path_arc=PI / 2
            )
        )
        self.wait(1)
