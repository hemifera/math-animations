...
class cpb_01(Scene):
    def construct(self):
        f = [
            MathTex(
                r"\begin{cases} (a+b)^{3}=a^{3}+3a^{2}b+3ab^{2}+b^{3} \\ (a-b)^{3}=a^{3}-3a^{2}b+3ab^{2}-b^{3} \end{cases}"
            ),
        ]
...
class cpb_02(Scene):
    def construct(self):
        ...
        self.wait(1)
        self.play(
            f[0].animate.shift(UP * 2), TransformMatchingShapes(f[0].copy(), f[2])
        )
        self.wait(1)
        self.play(Write(f[1].next_to(f[2], DOWN * 2)))
        self.wait(1)

        self.play(TransformMatchingShapes(f[2], f[3].move_to(f[2])))
        self.remove(f[2])
        self.play(f[3].animate.move_to(ORIGIN))
        self.wait(1)
        self.remove(f[1])
        for i in range(3, 5):
            self.play(ReplacementTransform(f[i], f[i + 1].move_to(f[i])))
            self.wait(1)
        self.wait(2)
...
class cpb_03(Scene):
    def construct(self):
        f = [
            ...
            MathTex(
                r"(5j^{9})^{3}+3(5j^{9})^{2}(13k^{4})+3(5j^{9})(13k^{4})^{2}+(13k^{4})^{3}"
            ),
...
class cpb_04(Scene):
    def construct(self):
        f = [
            ...
            MathTex(
                r"\begin{cases} a^{3} = 8u^{15}  \\ b^{3} =729 m^{3}p^{6} \end{cases}"
            ),
            MathTex(
                r"\begin{cases} \sqrt[3]{ a^{3} } = \sqrt[3]{ 8u^{15} }  \\ \sqrt[3]{ b^{3} } =\sqrt[3]{ 729 m^{3}p^{6} } \end{cases}"
            ),
            MathTex(
                r"\begin{cases} a = 2u^{15/3} \\ b = 9 m^{3/3} p^{6/3} \end{cases}"
            ),
        ...
        self.wait(1)
        self.play(f[0].animate.shift(UP * 2), Write(f[1].shift(DOWN * 2)))
        self.wait(2)
        self.play(TransformMatchingShapes(f[0], f[2].move_to(f[0])))
        self.wait(1)

        self.play(Write(f[3], run_time=0.7))
        self.wait(1)
        for i in range(3, 6):
            self.play(ReplacementTransform(f[i], f[i + 1].move_to(f[i])))
            self.wait(1)
