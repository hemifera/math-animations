...
class tcp_03(Scene):
    def construct(self):
        ...
        self.play(
            TransformMatchingTex(f[0].copy(), v0),
            Wait(2),
            FadeIn(v1)
        ...
        self.play(
            TransformMatchingShapes(
                Group(f[4], f[6]), Group(f[7], f[8]),
                key_map={r"a^{2}": r"a", r"b^{2}": r"b"}
            ),
            TransformMatchingShapes(
                Group(f[1], f[3]), Group(f[9], f[10]),
                key_map={f[9].tex_string: f[1].tex_string, f[3].tex_string: f[10].tex_string}
            )
        )
        self.wait(2)
        self.play(
        ...
        self.play(
            ...
            TransformMatchingTex(Group(f[11].copy(), f[12].copy()), f[13].shift(DOWN*2))
        )
        self.wait(2)
        self.play(
            TransformMatchingShapes(f[13], f[2], path_along_arc=PI/2)
        )
        self.wait(2)
        

        self.play(
            TransformMatchingTex(Group(f[11].copy(), f[12].copy()), f[14].shift(DOWN*2))
        )
        self.wait(2)
        self.play(
    ...
    def construct(self):
        ...
        for i, eq in enumerate(f[1:-1]):
            self.play(
                ...
                TransformMatchingShapes(f[i+1].copy(), f[i+2], path_along_arc=PI/2))
            self.wait(2)
