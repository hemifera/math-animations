...
class trig_10(Scene):
    def construct(self):
        ...
        ax = always_redraw(lambda: Axes(
            ...
            tips=False,))
        pln = always_redraw(lambda: NumberPlane(
        ...
        circle = always_redraw(lambda: Circle(radius = np.linalg.norm(ax.c2p(radius.get_value(), 0)-ax.c2p(0, 0))).move_to(ax.c2p(0,0)))
        ...
        radText = always_redraw(lambda: MathTex(rf"\text{{radio: }}{round(radius.get_value(), 2)}").shift(DOWN).shift(LEFT*4.5)) 
        brh = always_redraw(lambda: Brace(line1, sharpness=1, direction=line1.copy().rotate(PI / 2).get_unit_vector()))
        brhTex = always_redraw(lambda: brh.get_tex(r"\text{radio}"))

        theta_label = always_redraw(lambda: MathTex(rf"\theta: {round(theta.get_value(), 1)}^\circ").next_to(radText, DOWN))
        ...
        angle_label = always_redraw(lambda: MathTex(rf"{round(theta.get_value(), 1)}^\circ", ).move_to(angle_label_condition(theta.get_value())))
        ...
        self.wait()
        dot2 = always_redraw(lambda: Dot(point= ax.c2p(radius.get_value() * np.cos(theta.get_value() * DEGREES),0)))
        ly = always_redraw(lambda: Line(np.array([dot1.get_x(),0,0]),dot1.get_center(),color=YELLOW))
        self.play(FadeIn(dot2, ly))
        self.wait()
        

        bry = always_redraw(lambda: Brace(ly, sharpness=1, direction=ly.copy().rotate(-PI / 2).get_unit_vector()))
        bryTex = always_redraw(lambda: brh.get_tex(r"\text{distancia}\\ \text{vertical}").next_to(bry, ax.c2p(*RIGHT)))
        self.play(FadeIn(bry, bryTex))
        self.wait()
        self.play(Circumscribe(dot0, Circle), Circumscribe(dot2, Circle))
        self.wait()
        lx = always_redraw(lambda: Line(ax.c2p(*ORIGIN),np.array([dot1.get_x(),0, 0]),color=YELLOW))
        self.play(FadeIn(lx))

        brx = always_redraw(lambda: Brace(lx, sharpness=1, direction=lx.copy().rotate(-PI / 2).get_unit_vector()))
        brxTex = always_redraw(lambda: brh.get_tex(r"\text{distancia horizontal}").next_to(brx, DOWN))

        self.play(FadeIn(brx, brxTex))
        self.wait()
        
        x_label = always_redraw(lambda: MathTex(r"{}", round(radius.get_value() * np.cos(theta.get_value()*(PI/180)), 3))
            .move_to(
            np.array([
                dot1.get_x()/2,
                ax.c2p(0, -0.3)[1],
        ...
        y_label = always_redraw(lambda: MathTex(r"{}", round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3))
            .move_to(
            np.array([
                dot1.get_x()+ax.c2p(0.4, 0)[0],
                dot1.get_y()/2,
                0
            ])))
        radY = always_redraw(lambda: MathTex(rf"\text{{d. V: }}{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}").next_to(theta_label, DOWN))
        radX = always_redraw(lambda: MathTex(rf"\text{{d. H: }}{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}").next_to(radY, DOWN))
        self.play(
            FadeOut(bry, brx), 
...
radText = always_redraw(lambda: MathTex(rf"\text{{radio: }}{round(radius.get_value(), 2)}").shift(DOWN).shift(LEFT*5)) 
brh = always_redraw(lambda: Brace(line1, sharpness=1, direction=line1.copy().rotate(PI / 2).get_unit_vector()))
brhTex = always_redraw(lambda: brh.get_tex(r"\text{radio}"))
...
class trig_11(Scene):
    def construct(self):
        ...
        self.wait()
        brh1 = always_redraw(lambda: Brace(line1, sharpness=1, direction=line1.copy().rotate(PI / 2).get_unit_vector()))
        brhTex1 = always_redraw(lambda: brh1.get_tex(r"\text{hipotenusa}"))

        self.play(FadeIn(brh1, brhTex1))
        
        self.wait()
        radText1 = always_redraw(lambda: MathTex(rf"\text{{hipotenusa: }}{round(radius.get_value(), 2)}").move_to(radText)) 

        self.play(ReplacementTransform(radText, radText1))
        self.wait()
        self.play(theta_label.animate.next_to(radText1, DOWN))
        self.wait()
        self.theta_label = always_redraw(lambda: MathTex(rf"\theta: {round(theta.get_value(), 1)}^\circ").next_to(radText1, DOWN))
        

        bry1 = always_redraw(lambda: Brace(ly, sharpness=1, direction=ly.copy().rotate(-PI / 2).get_unit_vector()))
        bryTex1 =  bry1.get_tex(r"\text{cateto}\\ \text{opuesto}").next_to(bry1, RIGHT, buff=0.5)

        radY1 = always_redraw(lambda: MathTex(rf"\text{{c. opuesto: }}{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}").next_to(theta_label, DOWN))
        self.play(FadeIn(bry1, bryTex1), )
        self.wait()
        self.play(ReplacementTransform(radY, radY1))
        self.wait()

        brx1 = always_redraw(lambda: Brace(lx, sharpness=1, direction=lx.copy().rotate(-PI / 2).get_unit_vector()))
        brxTex1 = brx1.get_tex(r"\text{cateto adyacente}").next_to(brx1, DOWN)

        radX1 = always_redraw(lambda: MathTex(rf"\text{{c. adyacente: }}{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}").next_to(radY, DOWN))
        ...
        self.y_label = always_redraw(lambda: MathTex(r"{}", round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3))
            .move_to(
            np.array([
                dot1.get_x()+ax.c2p(0.2, 0)[0],
                dot1.get_y()/2,
                0
            ])))
        ...
        t00 = always_redraw(lambda: MathTex(rf"\tan ({round(theta.get_value(), 1)}^\circ)").shift(UP).shift(LEFT*5))

        
        self.play(TransformFromCopy(angle_label, t00))
        # self.play(FadeIn(t00))
        self.wait(1)
        t01 = always_redraw(lambda: MathTex(rf"=\frac{{{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}}}").next_to(t00, RIGHT, buff=0.25))
        self.play(TransformFromCopy(VGroup(y_label, x_label),t01))
        self.wait(1)

        t1 = always_redraw(lambda: MathTex(rf"={round(np.tan(theta.get_value() * (PI/180)), 3)}").next_to(t01, DOWN))
        self.play(FadeIn(t1))
        self.wait()
        ...
        t100 = always_redraw(lambda: MathTex(rf"\arctan \left( \frac{{{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}}} \right)").next_to(t00, UP*2.5))
        self.play(TransformFromCopy(t01, t100))
        self.wait()
        
        t12 = always_redraw(lambda: MathTex(rf"={round(theta.get_value(), 1)}^\circ").next_to(t100, RIGHT, buff=0.25))
        ...
        fu10 = MathTex(r"\tan (\theta) = \frac{\text{c. opuesto}}{\text{c. adyacente}}").next_to(x_label, DOWN)
        fu11 = MathTex(r"\arctan \left( \frac{\text{c. opuesto}}{\text{c. adyacente}}\right) = \theta").next_to(fu10, DOWN)
        ...
        def calc_hipotenuse_position(phi: float, mobject_reference):
            return ax.c2p(
                mobject_reference.get_center()[0] + (radius.get_value() / 2) *np.cos( (phi + 20) * (PI/180) ),
                mobject_reference.get_center()[1] + (radius.get_value() / 2) *np.sin( (phi + 20) * (PI/180) )
            )

        h_label = always_redraw(lambda: MathTex(rf"{round(radius.get_value(), 2)}").move_to(calc_hipotenuse_position(theta.get_value(), circle)))

        self.play(TransformFromCopy(radText1, h_label))
        self.wait(1)


        t20 = always_redraw(lambda: MathTex(rf"\sin ({round(theta.get_value(), 1)}^\circ)").shift(UP).shift(LEFT*5))
        self.play(TransformFromCopy(theta_label, t20))
        self.wait()

        
        
        t21 = always_redraw(lambda: MathTex(rf"=\frac{{{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value(), 2)}}}").next_to(t20, RIGHT, buff=0.25))
        self.play(TransformFromCopy(VGroup(h_label, y_label), t21))
        self.wait()
        t22 = always_redraw(lambda: MathTex(rf"={round(np.sin(theta.get_value() * (PI/180)), 3)}").next_to(t21, DOWN))
        self.play(Write(t22))

        self.wait()

        t23 = always_redraw(lambda: MathTex(rf"\arcsin \left( \frac{{{round(radius.get_value()*np.sin(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value(), 2)}}} \right)").next_to(t20, UP*2.5))
        self.play(TransformFromCopy(VGroup(h_label, y_label), t23))
        self.wait()
        t24 = always_redraw(lambda: MathTex(rf"={round(theta.get_value(), 1)}^\circ").next_to(t23, RIGHT, buff=0.25))
        ...
        self.play(radius.animate.set_value(1.5), rate_func=ease_in_out_cubic, run_time=1)
        self.wait(1)
        ...
        fu20 = MathTex(r"\sin (\theta) = \frac{\text{c. opuesto}}{\text{hipotenusa}}").next_to(x_label, DOWN)
        fu21 = MathTex(r"\arcsin \left(\frac{\text{c. opuesto}}{\text{hipotenusa}}\right) = \theta").next_to(fu20, DOWN)
        ...
        t30 = always_redraw(lambda: MathTex(rf"\cos ({round(theta.get_value(), 1)}^\circ)").shift(UP).shift(LEFT*5))
        self.play(TransformFromCopy(theta_label, t30))
        self.wait()

        
        
        t31 = always_redraw(lambda: MathTex(rf"=\frac{{{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value(), 2)}}}").next_to(t30, RIGHT, buff=0.25))
        self.play(TransformFromCopy(VGroup(h_label, x_label), t31))
        self.wait()
        t32 = always_redraw(lambda: MathTex(rf"={round(np.cos(theta.get_value() * (PI/180)), 3)}").next_to(t31, DOWN))
        self.play(Write(t32))

        self.wait()

        t33 = always_redraw(lambda: MathTex(rf"\arccos \left( \frac{{{round(radius.get_value()*np.cos(theta.get_value()* (PI/180)), 3)}}}{{{round(radius.get_value(), 2)}}} \right)").next_to(t30, UP*2.5))
        self.play(TransformFromCopy(VGroup(h_label, y_label), t33))
        self.wait()
        t34 = always_redraw(lambda: MathTex(rf"={round(theta.get_value(), 1)}^\circ").next_to(t33, RIGHT, buff=0.25))
        ...
        fu30 = MathTex(r"\cos (\theta) = \frac{\text{c. adyacente}}{\text{hipotenusa}}").next_to(x_label, DOWN)
        fu31 = MathTex(r"\arccos \left(\frac{\text{c. adyacente}}{\text{hipotenusa}}\right) = \theta").next_to(fu30, DOWN)
...
class trig_13(Scene):
    def construct(self):
        ...
        f = [
            ...
            MathTex(r"\arcsin \left(\frac{\text{c. opuesto}}{\text{hipotenusa}}\right) = \theta "),

            MathTex(r"\cos \theta = \frac{\text{c. adyacente}}{\text{hipotenusa}}"),
            MathTex(r"\arccos \left(\frac{\text{c. adyacente}}{\text{hipotenusa}}\right) = \theta "),

            MathTex(r"\tan \theta = \frac{\text{c. opuesto}}{\text{c. adyacente}} = \frac{\sin \theta}{\cos \theta}"),
            MathTex(r"\arctan \left(\frac{\text{c. opuesto}}{\text{c. adyacente}}\right) = \theta "),



            MathTex(r"\csc \theta = \frac{\text{hipotenusa}}{\text{c. opuesto}} = \frac{1}{\sin \theta}"),
            MathTex(r"\operatorname{arccsc} \left(\frac{\text{hipotenusa}}{\text{c. opuesto}}\right) = \theta "),

            MathTex(r"\sec \theta = \frac{\text{hipotenusa}}{\text{c. adyacente}} = \frac{1}{\cos \theta}"),
            MathTex(r"\operatorname{arcsec} \left(\frac{\text{hipotenusa}}{\text{c. adyacente}}\right) = \theta "),

            MathTex(r"\cot \theta = \frac{\text{c. adyacente}}{\text{c. opuesto}} = \frac{1}{\tan \theta}"),
            MathTex(r"\operatorname{arccot} \left(\frac{\text{c. adyacente}}{\text{c. opuesto}}\right) = \theta "),
...
class trig_14(Scene):
    def construct(self):
        ...
        dot0 = Dot(LEFT*0.5)
        dot1 = Dot()
        dot1.add_updater(lambda m: m.move_to([
            dot0.get_center()[0] + radius.get_value() * np.cos(theta.get_value() * DEGREES),
            dot0.get_center()[1],
            0
        ]))

        dot2 = Dot()
        dot2.add_updater(lambda m: m.move_to([
            dot0.get_center()[0] + radius.get_value() * np.cos(theta.get_value() * DEGREES),
            dot0.get_center()[1] + radius.get_value() * np.sin(theta.get_value() * DEGREES),
            0
        ]))
        ...
        lh = always_redraw(lambda: Line(start=dot0.get_center(), end=dot2.get_center(), color=YELLOW))
        lx = always_redraw(lambda: Line(start=dot0.get_center(), end=dot1.get_center(), color=YELLOW))
        ly = always_redraw(lambda: Line(start=dot1.get_center(), end=dot2.get_center(), color=YELLOW))
        ...
        a0T = always_redraw(
            lambda: DecimalNumber(
                ...
                unit=r"^{\circ}").move_to([
                    dot0.get_center()[0] + 0.4 * radius.get_value() * np.cos((theta.get_value()/2) * DEGREES),
                    dot0.get_center()[1] + 0.4 * radius.get_value() * np.sin((theta.get_value()/2) * DEGREES),
                    0]
                )
            )

        a1 = always_redraw(lambda: Angle(lh.copy().reverse_points(), ly.copy().reverse_points(), radius= 0.2 * radius.get_value(), other_angle=False))

        a1T = always_redraw(
            lambda: DecimalNumber(
                Angle(lh.copy().reverse_points(), ly.copy().reverse_points(), other_angle=False).get_value(degrees=True),  # <— calcular sobre la marcha
                unit=r"^{\circ}",).move_to([
                    dot2.get_center()[0] - 0.3 * radius.get_value() * np.sin(((90 - theta.get_value())/2) * DEGREES),
                    dot2.get_center()[1] - 0.3 * radius.get_value() * np.cos(((90-theta.get_value())/2) * DEGREES),
                    0]
                )
            )
        ...
        def calc_post(phi: float, radius: float, mobjectA: Mobject, mobjectB: Mobject):
            return np.array([
                mobjectA.get_center()[0] - np.linalg.norm(mobjectB.get_center() - mobjectB.get_right()) + (radius / 2) * np.cos( (phi + 20) * DEGREES ),
                mobjectA.get_center()[1] + (radius / 2) * np.sin( (phi + 20) * DEGREES ),
                0])


        brx = always_redraw(lambda: Brace(lx, sharpness=1, direction=lx.copy().rotate(-PI / 2).get_unit_vector()))
        brxTex = always_redraw(lambda: MathTex(r"\text{c. adyacente}").next_to(brx, DOWN))

        bry = always_redraw(lambda: Brace(ly, sharpness=1, direction=ly.copy().rotate(-PI / 2).get_unit_vector()))
        bryTex = always_redraw(lambda: MathTex(r"\text{c. opuesto}").next_to(bry, RIGHT))

        brh = always_redraw(lambda: Brace(lh, sharpness=1, direction=lh.copy().rotate(PI / 2).get_unit_vector()))
        brhTex = MathTex(r"5.28")
        brhTex.add_updater(lambda k: k.move_to(calc_post(
            theta.get_value(),
            radius.get_value(),
            dot0,
            brhTex)))
        ...
        self.play(VGroup(f[0], f[1]).animate.arrange(DOWN, buff=0.5).shift(UP*2).shift(LEFT*4), dot0.animate.shift(RIGHT).shift(DOWN))
        self.wait()
        t0 = [
        ...
        self.play(theta.animate.set_value(53.13), rate_func=ease_in_out_quint, run_time=2)
        self.wait()
        ...
        t36 = MathTex(r"36.87 ^\circ").move_to(dot2.get_center()).shift(DOWN*2 + LEFT*0.7)
        ...
        for i in range(4):
            if i == 1:
                self.play(TransformMatchingShapes(VGroup(f1[i], brxTex2.copy(), bryTex2.copy()), f1[i+1],path_arc=PI/2))
        ...
        t25 = MathTex(r"25 ^\circ").move_to(dot2.get_center()).shift(DOWN*2 + LEFT*0.7)
        t65 = MathTex(r"65 ^\circ").move_to(dot0).shift(UP*0.5+RIGHT*1.7)
        
        brh3 = Brace(lh, sharpness=1, direction=lh.copy().rotate(PI / 2).get_unit_vector())
        brhT = Tex(r"hipotenusa").move_to(brh3.get_center()).shift(LEFT*2).shift(UP)
        
        brx3 = Brace(lx, sharpness=1, direction=lx.copy().rotate(-PI / 2).get_unit_vector())
        brxT = Tex(r"c. opuesto").move_to(brx3.get_center()).shift(DOWN)
        
        bry3 = Brace(ly, sharpness=1, direction=ly.copy().rotate(-PI / 2).get_unit_vector())
        bry3T = Tex(r"c. adyacente").move_to(bry3.get_center()).shift(RIGHT*1.5)
        
        
        yyyy = MathTex(r"12.25").move_to(bry3.get_center()).shift(RIGHT)

        t25 = MathTex(r"25 ^\circ").move_to(dot2.get_center()).shift(DOWN*1.5 + LEFT*0.25)
        ...
        self.play(VGroup(f[1], f[0]).animate.arrange(DOWN, buff=0.5).shift(UP*2).shift(LEFT*4), )
        self.wait()
        # self.play()
